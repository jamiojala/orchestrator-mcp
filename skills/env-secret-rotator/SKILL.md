...
prompt_template: |
  ...
  ## Your Task
  Use the supplied code, architecture, or product context to plan and stage zero-downtime secret rotation across services that currently share brittle static credentials.
  ...
  - Validation plan covering `verify_rotation_safety`.
  - Include the most likely failure modes, operator notes, and composition boundaries with adjacent systems or skills.
  
  ## Validation Checklist
  - Ensure `verify_rotation_safety` passes or explain why it cannot run
---
# Env Secret Rotator

Superpower: Plan and stage zero-downtime secret rotation across services that currently share brittle static credentials.
...
## Use this skill when
- The request signals `secret rotation` or an equivalent domain problem.
- The request signals `key rollover` or an equivalent domain problem.
- The request signals `credential refresh` or an equivalent domain problem.
- The likely implementation surface includes `**/.env*`.
...
## Handoff notes
- Treat ``verify_rotation_safety`` as the minimum proof surface before calling the work complete.
