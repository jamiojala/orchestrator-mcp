...
prompt_template: |
  ...
  ## Your Task
  Use the supplied code, architecture, or product context to shrink container builds and speed up rebuilds through smarter layer ordering and multi-stage decomposition.
  ...
  - Ensure `verify_image_size` passes or explain why it cannot run
---
...
## Use this skill when
- The request signals `docker layer` or an equivalent domain problem.
- The request signals `multi stage` or an equivalent domain problem.
- The request signals `image size` or an equivalent domain problem.
- The likely implementation surface includes `**/Dockerfile*`.
- The likely implementation surface includes `**/.dockerignore`.
...
## Handoff notes
- Treat ``verify_image_size`` as the minimum proof surface before calling the work complete.
