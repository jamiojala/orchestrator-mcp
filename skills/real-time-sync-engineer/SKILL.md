...
prompt_template: |
  ...
  ## Your Task
  Use the supplied code, architecture, or product context to add optimistic ui, sync reconciliation, and offline-safe conflict handling to collaborative product flows.
  ...
  - Validation plan covering `verify_offline_behavior`.
  - Include the most likely failure modes, operator notes, and composition boundaries with adjacent systems or skills.
  
  ## Validation Checklist
  - Ensure `verify_offline_behavior` passes or explain why it cannot run
---
# Real-Time Sync Engineer

Superpower: Add optimistic UI, sync reconciliation, and offline-safe conflict handling to collaborative product flows.

## Use this skill when
- The request signals `realtime` or an equivalent domain problem.
- The request signals `optimistic ui` or an equivalent domain problem.
- The request signals `conflict resolution` or an equivalent domain problem.
- The likely implementation surface includes `**/*.tsx`.
- The likely implementation surface includes `**/*realtime*.ts`.
- The likely implementation surface includes `**/*socket*.ts`.
...
## Handoff notes
- Treat ``verify_offline_behavior`` as the minimum proof surface before calling the work complete.
