...
prompt_template: |
  ...
  ## Your Task
  Use the supplied code, architecture, or product context to build sub-second analytics pipelines over streaming events without turning the system into an operational mystery.
  ...
  - Ensure `verify_latency_metrics` passes or explain why it cannot run
---
...
## Use this skill when
- The request signals `real time analytics` or an equivalent domain problem.
- The request signals `clickhouse` or an equivalent domain problem.
- The request signals `streaming` or an equivalent domain problem.
- The likely implementation surface includes `**/*.sql`.
- The likely implementation surface includes `**/stream/**`.
...
## Handoff notes
- Treat ``verify_latency_metrics`` as the minimum proof surface before calling the work complete.
