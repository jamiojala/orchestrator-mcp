---
name: Data Migration Blueprinter
...
prompt_template: |
  ...
  ## Your Task
  Use the supplied code, architecture, or product context to design zero-downtime data migrations with rollback choreography, phased reads, and production-safe cutovers.
  ...
  - Validation plan covering `verify_migration_safety`.
  - Include the most likely failure modes, operator notes, and composition boundaries with adjacent systems or skills.
  
  ## Validation Checklist
  - Ensure `verify_migration_safety` passes or explain why it cannot run
---
# Data Migration Blueprinter

Superpower: Design zero-downtime data migrations with rollback choreography, phased reads, and production-safe cutovers.

## Use this skill when
- The request signals `data migration` or an equivalent domain problem.
- The request signals `rollback` or an equivalent domain problem.
- The request signals `zero downtime` or an equivalent domain problem.
- The likely implementation surface includes `**/migrations/**`.
...
## Handoff notes
- Treat ``verify_migration_safety`` as the minimum proof surface before calling the work complete.
