...
prompt_template: |
  ...
  ## Your Task
  Use the supplied code, architecture, or product context to introduce feature-flag systems with kill switches, experiment hooks, and isolated rollout boundaries.
  ...
  - Validation plan covering `verify_flag_isolation`.
  - Include the most likely failure modes, operator notes, and composition boundaries with adjacent systems or skills.
  
  ## Validation Checklist
  - Ensure `verify_flag_isolation` passes or explain why it cannot run
---
# Feature-Flag Architect

Superpower: Introduce feature-flag systems with kill switches, experiment hooks, and isolated rollout boundaries.
...
## Use this skill when
- The request signals `feature flag` or an equivalent domain problem.
- The request signals `kill switch` or an equivalent domain problem.
...
## Handoff notes
- Treat ``verify_flag_isolation`` as the minimum proof surface before calling the work complete.
