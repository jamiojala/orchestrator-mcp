...
prompt_template: |
  ...
  ## Your Task
  Use the supplied code, architecture, or product context to split long-running serverless logic into edge-safe, cold-start-aware execution paths with lower runtime cost.
  ...
  - Validation plan covering `audit_cold_start_metrics`.
  - Include the most likely failure modes, operator notes, and composition boundaries with adjacent systems or skills.
  
  ## Validation Checklist
  - Ensure `audit_cold_start_metrics` passes or explain why it cannot run
---
...
## Use this skill when
- The request signals `serverless cost` or an equivalent domain problem.
- The request signals `cold start` or an equivalent domain problem.
- The request signals `lambda` or an equivalent domain problem.
- The likely implementation surface includes `**/api/**/*.ts`.
- The likely implementation surface includes `**/*.lambda.*`.
- The likely implementation surface includes `**/functions/**`.
...
## Handoff notes
- Treat ``audit_cold_start_metrics`` as the minimum proof surface before calling the work complete.
