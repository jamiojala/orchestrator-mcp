...
prompt_template: |
  ...
  ## Your Task
  Use the supplied code, architecture, or product context to refactor ad hoc infrastructure into reusable, environment-safe terraform modules with clean variable boundaries.
  ...
  - Validation plan covering `verify_terraform_plan`.
  - Include the most likely failure modes, operator notes, and composition boundaries with adjacent systems or skills.
  
  ## Validation Checklist
  - Ensure `verify_terraform_plan` passes or explain why it cannot run
---
# Terraform Module Architect

Superpower: Refactor ad hoc infrastructure into reusable, environment-safe Terraform modules with clean variable boundaries.

## Use this skill when
- The request signals `terraform module` or an equivalent domain problem.
- The request signals `iac` or an equivalent domain problem.
- The request signals `environment overlay` or an equivalent domain problem.
...
## Handoff notes
- Treat ``verify_terraform_plan`` as the minimum proof surface before calling the work complete.
