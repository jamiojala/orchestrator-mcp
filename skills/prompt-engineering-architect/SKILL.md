...
prompt_template: |
  ...
  ## Your Task
  Use the supplied code, architecture, or product context to design system prompts, prompt contracts, and eval-backed example sets that improve llm reliability without hiding failure modes.
  ...
  - Validation plan covering `prompt-performance-checker`, `output-consistency-validator`, `token-usage-optimizer`.
  - Include the most likely failure modes, operator notes, and composition boundaries with adjacent systems or skills.
  
  ## Validation Checklist
  - Ensure `prompt-performance-checker` passes or explain why it cannot run
  - Ensure `output-consistency-validator` passes or explain why it cannot run
  - Ensure `token-usage-optimizer` passes or explain why it cannot run
---
# Prompt Engineering Architect

Superpower: Design system prompts, prompt contracts, and eval-backed example sets that improve LLM reliability without hiding failure modes.
...
## Use this skill when
- The request signals `system prompt` or an equivalent domain problem.
- The request signals `few shot` or an equivalent domain problem.
- The request signals `prompt contract` or an equivalent domain problem.
...
## Handoff notes
- Treat ``prompt-performance-checker`, `output-consistency-validator`, `token-usage-optimizer`` as the minimum proof surface before calling the work complete.
