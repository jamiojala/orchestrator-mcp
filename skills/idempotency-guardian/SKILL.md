...
prompt_template: |
  ...
  ## Your Task
  Use the supplied code, architecture, or product context to make mutation endpoints safely retryable with idempotency keys and conflict-aware persistence patterns.
  ...
  - Validation plan covering `verify_idempotency_guarantees`.
  - Include the most likely failure modes, operator notes, and composition boundaries with adjacent systems or skills.
  
  ## Validation Checklist
  - Ensure `verify_idempotency_guarantees` passes or explain why it cannot run
---
# Idempotency Guardian

Superpower: Make mutation endpoints safely retryable with idempotency keys and conflict-aware persistence patterns.
...
## Use this skill when
- The request signals `idempotency` or an equivalent domain problem.
- The request signals `safe retries` or an equivalent domain problem.
- The request signals `mutation api` or an equivalent domain problem.
- The likely implementation surface includes `**/api/**`.
- The likely implementation surface includes `**/payments/**`.
...
## Handoff notes
- Treat ``verify_idempotency_guarantees`` as the minimum proof surface before calling the work complete.
