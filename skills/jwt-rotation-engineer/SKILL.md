...
prompt_template: |
  ...
  ## Your Task
  Use the supplied code, architecture, or product context to modernize session flows with rotating refresh tokens, revocation, and secure cookie handling.
  ...
  - Ensure `verify_token_security` passes or explain why it cannot run
---
...
## Use this skill when
- The request signals `jwt` or an equivalent domain problem.
- The request signals `refresh token` or an equivalent domain problem.
- The request signals `httpOnly cookie` or an equivalent domain problem.
- The likely implementation surface includes `**/auth/**`.
...
## Handoff notes
- Treat ``verify_token_security`` as the minimum proof surface before calling the work complete.
