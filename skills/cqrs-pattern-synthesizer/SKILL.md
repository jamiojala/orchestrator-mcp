...
prompt_template: |
  ...
  ## Your Task
  Use the supplied code, architecture, or product context to separate read and write models into typed cqrs flows with query optimization and command safety.
...
## Use this skill when
- The request signals `cqrs` or an equivalent domain problem.
- The request signals `command handler` or an equivalent domain problem.
- The request signals `query model` or an equivalent domain problem.
...
## Handoff notes
- Treat ``verify_type_consistency`` as the minimum proof surface before calling the work complete.
