...
prompt_template: |
  ...
  ## Your Task
  Use the supplied code, architecture, or product context to identify interactive islands in otherwise static pages and hydrate only what truly needs runtime code.
  ...
  - Validation plan covering `verify_hydration_bundles`.
  - Include the most likely failure modes, operator notes, and composition boundaries with adjacent systems or skills.
  
  ## Validation Checklist
  - Ensure `verify_hydration_bundles` passes or explain why it cannot run
---
# SSG Island Architect

Superpower: Identify interactive islands in otherwise static pages and hydrate only what truly needs runtime code.
...
## Use this skill when
- The request signals `islands architecture` or an equivalent domain problem.
- The request signals `partial hydration` or an equivalent domain problem.
- The request signals `ssg` or an equivalent domain problem.
- The likely implementation surface includes `**/*.astro`.
...
## Handoff notes
- Treat ``verify_hydration_bundles`` as the minimum proof surface before calling the work complete.
