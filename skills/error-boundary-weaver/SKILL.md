...
prompt_template: |
  ...
  ## Your Task
  Use the supplied code, architecture, or product context to wrap interface trees with resilient error boundaries, fallback recovery, and observability-friendly failure paths.
...
## Use this skill when
- The request signals `error boundary` or an equivalent domain problem.
- The request signals `fallback ui` or an equivalent domain problem.
- The request signals `sentry` or an equivalent domain problem.
...
## Handoff notes
- Treat ``verify_error_reporting`` as the minimum proof surface before calling the work complete.
