...
prompt_template: |
  ...
  ## Your Task
  Use the supplied code, architecture, or product context to build evaluation loops for ai systems with benchmark sets, rubric design, judge calibration, and human-review anchors.
  ...
  - Ensure `evaluation-coverage-checker` passes or explain why it cannot run
  - Ensure `judge-calibration-validator` passes or explain why it cannot run
  - Ensure `benchmark-completeness-test` passes or explain why it cannot run
---
...
## Use this skill when
- The request signals `ai evaluation` or an equivalent domain problem.
- The request signals `benchmark suite` or an equivalent domain problem.
- The request signals `llm judge` or an equivalent domain problem.
...
## Handoff notes
- Treat ``evaluation-coverage-checker`, `judge-calibration-validator`, `benchmark-completeness-test`` as the minimum proof surface before calling the work complete.
