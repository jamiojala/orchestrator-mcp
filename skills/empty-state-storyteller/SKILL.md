...
prompt_template: |
  ...
  ## Your Task
  Use the supplied code, architecture, or product context to turn empty and null states into contextual guidance that helps users reach value instead of dead ends.
  ...
  - Validation plan covering `verify_engagement_metrics`.
  - Include the most likely failure modes, operator notes, and composition boundaries with adjacent systems or skills.
  
  ## Validation Checklist
  - Ensure `verify_engagement_metrics` passes or explain why it cannot run
---
# Empty State Storyteller

Superpower: Turn empty and null states into contextual guidance that helps users reach value instead of dead ends.
...
## Use this skill when
- The request signals `empty state` or an equivalent domain problem.
- The request signals `cta` or an equivalent domain problem.
- The request signals `null state` or an equivalent domain problem.
...
## Handoff notes
- Treat ``verify_engagement_metrics`` as the minimum proof surface before calling the work complete.
