...
prompt_template: |
  ...
  - Ensure `verify_route_integrity` passes or explain why it cannot run
---
...
- The request signals `route integrity` or an equivalent domain problem.
- The likely implementation surface includes `**/*.tsx`.
- The likely implementation surface includes `**/vite.config.*`.
- The likely implementation surface includes `**/webpack*.js`.
...
## Handoff notes
- Treat ``audit_bundle_size`, `verify_route_integrity`` as the minimum proof surface before calling the work complete.
