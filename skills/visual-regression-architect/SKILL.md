...
prompt_template: |
  ...
  ## Your Task
  Use the supplied code, architecture, or product context to set up screenshot-based ui regression pipelines that catch real layout and styling breakage without noise.
  ...
  - Ensure `verify_screenshot_consistency` passes or explain why it cannot run
---
...
## Use this skill when
- The request signals `visual regression` or an equivalent domain problem.
- The request signals `playwright` or an equivalent domain problem.
- The request signals `chromatic` or an equivalent domain problem.
- The likely implementation surface includes `**/*.tsx`.
- The likely implementation surface includes `**/playwright*.ts`.
...
## Handoff notes
- Treat ``verify_screenshot_consistency`` as the minimum proof surface before calling the work complete.
