...
prompt_template: |
  ...
  - Ensure `verify_type_coverage` passes or explain why it cannot run
---
...
- The request signals `typed icons` or an equivalent domain problem.
- The likely implementation surface includes `**/*.svg`.
- The likely implementation surface includes `**/icons/**`.
...
## Handoff notes
- Treat ``verify_type_coverage`` as the minimum proof surface before calling the work complete.
