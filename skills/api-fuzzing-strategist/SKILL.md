...
prompt_template: |
  ...
  ## Your Task
  Use the supplied code, architecture, or product context to build property-based and boundary-oriented api tests that find input handling failures before attackers or customers do.
  ...
  - Ensure `verify_boundary_coverage` passes or explain why it cannot run
---
...
## Use this skill when
- The request signals `api fuzzing` or an equivalent domain problem.
- The request signals `property based` or an equivalent domain problem.
- The request signals `boundary values` or an equivalent domain problem.
...
## Handoff notes
- Treat ``verify_boundary_coverage`` as the minimum proof surface before calling the work complete.
