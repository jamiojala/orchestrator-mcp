...
prompt_template: |
  ...
  ## Your Task
  Use the supplied code, architecture, or product context to optimize model serving with batching, quantization, streaming, and deployment-aware latency budgets that preserve quality.
  ...
  - Validation plan covering `inference-latency-checker`, `throughput-validator`, `accuracy-impact-test`.
  - Include the most likely failure modes, operator notes, and composition boundaries with adjacent systems or skills.
  
  ## Validation Checklist
  - Ensure `inference-latency-checker` passes or explain why it cannot run
  - Ensure `throughput-validator` passes or explain why it cannot run
  - Ensure `accuracy-impact-test` passes or explain why it cannot run
---
# Inference Optimization Engineer

Superpower: Optimize model serving with batching, quantization, streaming, and deployment-aware latency budgets that preserve quality.
...
## Use this skill when
- The request signals `quantization` or an equivalent domain problem.
- The request signals `batching` or an equivalent domain problem.
- The request signals `inference latency` or an equivalent domain problem.
- The likely implementation surface includes `**/*.py`.
- The likely implementation surface includes `**/*.cpp`.
- The likely implementation surface includes `**/*.onnx`.
...
## Handoff notes
- Treat ``inference-latency-checker`, `throughput-validator`, `accuracy-impact-test`` as the minimum proof surface before calling the work complete.
