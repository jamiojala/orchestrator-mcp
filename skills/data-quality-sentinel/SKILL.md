...
prompt_template: |
  ...
  ## Your Task
  Use the supplied code, architecture, or product context to instrument data pipelines with freshness, completeness, and anomaly detection checks that fail usefully.
...
## Use this skill when
- The request signals `great expectations` or an equivalent domain problem.
- The request signals `data quality` or an equivalent domain problem.
- The request signals `anomaly detection` or an equivalent domain problem.
- The likely implementation surface includes `**/*.py`.
- The likely implementation surface includes `**/etl/**`.
...
## Handoff notes
- Treat ``verify_data_freshness`` as the minimum proof surface before calling the work complete.
