...
prompt_template: |
  ...
  ## Your Task
  Use the supplied code, architecture, or product context to design realistic load scenarios with useful bottleneck signals rather than synthetic throughput vanity metrics.
...
## Use this skill when
- The request signals `load test` or an equivalent domain problem.
- The request signals `k6` or an equivalent domain problem.
- The request signals `traffic simulation` or an equivalent domain problem.
- The likely implementation surface includes `**/api/**`.
- The likely implementation surface includes `**/k6/**`.
- The likely implementation surface includes `**/artillery/**`.
...
## Handoff notes
- Treat ``verify_load_thresholds`` as the minimum proof surface before calling the work complete.
