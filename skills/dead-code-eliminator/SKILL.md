...
prompt_template: |
  ...
  ## Your Task
  Use the supplied code, architecture, or product context to find and remove unused code paths, exports, and stale branches that still carry maintenance cost and risk.
  ...
  - Validation plan covering `verify_bundle_reduction`.
  - Include the most likely failure modes, operator notes, and composition boundaries with adjacent systems or skills.
  
  ## Validation Checklist
  - Ensure `verify_bundle_reduction` passes or explain why it cannot run
---
# Dead Code Eliminator

Superpower: Find and remove unused code paths, exports, and stale branches that still carry maintenance cost and risk.
...
## Use this skill when
- The request signals `dead code` or an equivalent domain problem.
- The request signals `unused exports` or an equivalent domain problem.
- The request signals `bundle cleanup` or an equivalent domain problem.
...
## Handoff notes
- Treat ``verify_bundle_reduction`` as the minimum proof surface before calling the work complete.
