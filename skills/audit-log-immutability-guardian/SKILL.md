---
name: Audit Log Immutability Guardian
...
prompt_template: |
  ...
  ## Your Task
  Use the supplied code, architecture, or product context to protect critical audit trails against tampering with append-only integrity and verification strategies.
  ...
  - Validation plan covering `verify_log_integrity`.
  - Include the most likely failure modes, operator notes, and composition boundaries with adjacent systems or skills.
  
  ## Validation Checklist
  - Ensure `verify_log_integrity` passes or explain why it cannot run
---
# Audit Log Immutability Guardian

Superpower: Protect critical audit trails against tampering with append-only integrity and verification strategies.
...
## Use this skill when
- The request signals `audit log` or an equivalent domain problem.
- The request signals `immutability` or an equivalent domain problem.
- The request signals `worm` or an equivalent domain problem.
...
## Handoff notes
- Treat ``verify_log_integrity`` as the minimum proof surface before calling the work complete.
