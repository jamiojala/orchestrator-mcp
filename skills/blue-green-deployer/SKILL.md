...
prompt_template: |
  ...
  ## Your Task
  Use the supplied code, architecture, or product context to automate safer production rollouts with health-aware promotion and rollback triggers.
  ...
  - Validation plan covering `verify_deployment_health`.
  - Include the most likely failure modes, operator notes, and composition boundaries with adjacent systems or skills.
  
  ## Validation Checklist
  - Ensure `verify_deployment_health` passes or explain why it cannot run
---
# Blue-Green Deployer

Superpower: Automate safer production rollouts with health-aware promotion and rollback triggers.
...
## Use this skill when
- The request signals `blue green` or an equivalent domain problem.
- The request signals `deployment rollback` or an equivalent domain problem.
- The request signals `release health` or an equivalent domain problem.
- The likely implementation surface includes `**/*.yaml`.
- The likely implementation surface includes `**/deploy/**`.
...
## Handoff notes
- Treat ``verify_deployment_health`` as the minimum proof surface before calling the work complete.
