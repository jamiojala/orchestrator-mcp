...
prompt_template: |
  ...
  ## Your Task
  Use the supplied code, architecture, or product context to integrate text, vision, audio, and document intelligence into one application surface with graceful modality-aware fallbacks.
  ...
  - Validation plan covering `modality-fusion-checker`, `latency-validator`, `quality-assessment-test`.
  - Include the most likely failure modes, operator notes, and composition boundaries with adjacent systems or skills.
  
  ## Validation Checklist
  - Ensure `modality-fusion-checker` passes or explain why it cannot run
  - Ensure `latency-validator` passes or explain why it cannot run
  - Ensure `quality-assessment-test` passes or explain why it cannot run
---
# Multimodal AI Integrator

Superpower: Integrate text, vision, audio, and document intelligence into one application surface with graceful modality-aware fallbacks.
...
## Use this skill when
- The request signals `multimodal` or an equivalent domain problem.
- The request signals `vision model` or an equivalent domain problem.
- The request signals `audio model` or an equivalent domain problem.
- The likely implementation surface includes `**/*.py`.
- The likely implementation surface includes `**/*.ts`.
- The likely implementation surface includes `**/vision/**`.
...
## Handoff notes
- Treat ``modality-fusion-checker`, `latency-validator`, `quality-assessment-test`` as the minimum proof surface before calling the work complete.
