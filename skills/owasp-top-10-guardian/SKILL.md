...
prompt_template: |
  ...
  ## Your Task
  Use the supplied code, architecture, or product context to systematically surface high-probability application security weaknesses across common owasp failure modes.
...
## Use this skill when
- The request signals `owasp` or an equivalent domain problem.
- The request signals `xss` or an equivalent domain problem.
- The request signals `csrf` or an equivalent domain problem.
...
## Handoff notes
- Treat ``verify_owasp_compliance`` as the minimum proof surface before calling the work complete.
