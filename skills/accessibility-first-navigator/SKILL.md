...
prompt_template: |
  ...
  ## Your Task
  Use the supplied code, architecture, or product context to make site and app navigation work cleanly for keyboard and screen-reader users without feeling like a compliance add-on.
  ...
  - Ensure `audit_screen_reader_compatibility` passes or explain why it cannot run
---
...
## Use this skill when
- The request signals `screen reader` or an equivalent domain problem.
- The request signals `skip link` or an equivalent domain problem.
- The request signals `navigation accessibility` or an equivalent domain problem.
- The likely implementation surface includes `**/*.tsx`.
- The likely implementation surface includes `**/navigation/**`.
...
## Handoff notes
- Treat ``audit_screen_reader_compatibility`` as the minimum proof surface before calling the work complete.
