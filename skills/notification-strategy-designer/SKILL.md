---
name: Notification Strategy Designer
...
prompt_template: |
  ...
  - Ensure `verify_opt_out_rates` passes or explain why it cannot run
---
...
- The request signals `engagement` or an equivalent domain problem.
- The likely implementation surface includes `**/*.ts`.
- The likely implementation surface includes `**/*.tsx`.
- The likely implementation surface includes `**/notifications/**`.
...
## Handoff notes
- Treat ``verify_opt_out_rates`` as the minimum proof surface before calling the work complete.
