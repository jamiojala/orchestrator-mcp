...
prompt_template: |
  ...
  ## Your Task
  Use the supplied code, architecture, or product context to wrap external api calls with circuit breakers, retries, fallbacks, and backoff while preserving business logic shape.
  ...
  - Ensure `verify_error_handling` passes or explain why it cannot run
---
...
## Use this skill when
- The request signals `circuit breaker` or an equivalent domain problem.
- The request signals `retry` or an equivalent domain problem.
- The request signals `backoff` or an equivalent domain problem.
...
## Handoff notes
- Treat ``verify_error_handling`` as the minimum proof surface before calling the work complete.
