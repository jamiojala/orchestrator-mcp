---
name: API Test Contract Enforcer
...
prompt_template: |
  ...
  ## Your Task
  Use the supplied code, architecture, or product context to set up contract testing so api consumers and providers fail safely before incompatible releases ship.
  ...
  - Validation plan covering `verify_contract_compliance`.
  - Include the most likely failure modes, operator notes, and composition boundaries with adjacent systems or skills.
  
  ## Validation Checklist
  - Ensure `verify_contract_compliance` passes or explain why it cannot run
---
# API Test Contract Enforcer

Superpower: Set up contract testing so API consumers and providers fail safely before incompatible releases ship.
...
## Use this skill when
- The request signals `contract test` or an equivalent domain problem.
- The request signals `pact` or an equivalent domain problem.
- The request signals `consumer provider` or an equivalent domain problem.
...
## Handoff notes
- Treat ``verify_contract_compliance`` as the minimum proof surface before calling the work complete.
