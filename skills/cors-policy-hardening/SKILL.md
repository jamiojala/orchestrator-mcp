...
prompt_template: |
  ...
  ## Your Task
  Use the supplied code, architecture, or product context to tighten cross-origin boundaries without silently breaking legitimate credentialed traffic.
  ...
  - Validation plan covering `verify_cors_strictness`.
  - Include the most likely failure modes, operator notes, and composition boundaries with adjacent systems or skills.
  
  ## Validation Checklist
  - Ensure `verify_cors_strictness` passes or explain why it cannot run
---
# CORS Policy Hardening

Superpower: Tighten cross-origin boundaries without silently breaking legitimate credentialed traffic.

## Use this skill when
- The request signals `cors` or an equivalent domain problem.
- The request signals `origin allowlist` or an equivalent domain problem.
- The request signals `credentials` or an equivalent domain problem.
...
## Handoff notes
- Treat ``verify_cors_strictness`` as the minimum proof surface before calling the work complete.
