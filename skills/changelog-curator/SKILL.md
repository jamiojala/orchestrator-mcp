...
prompt_template: |
  ...
  - Ensure `verify_changelog_accuracy` passes or explain why it cannot run
---
# Changelog Curator
...
- The request signals `conventional commits` or an equivalent domain problem.
- The likely implementation surface includes `**/.git/**`.
...
## Handoff notes
- Treat ``verify_changelog_accuracy`` as the minimum proof surface before calling the work complete.
