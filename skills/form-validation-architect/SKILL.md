...
prompt_template: |
  ...
  ## Your Task
  Use the supplied code, architecture, or product context to unify form ux around typed schemas, accessible feedback, and i18n-aware validation contracts.
  ...
  - Validation plan covering `verify_accessibility_labels`.
  - Include the most likely failure modes, operator notes, and composition boundaries with adjacent systems or skills.
  
  ## Validation Checklist
  - Ensure `verify_accessibility_labels` passes or explain why it cannot run
---
# Form Validation Architect

Superpower: Unify form UX around typed schemas, accessible feedback, and i18n-aware validation contracts.
...
## Use this skill when
- The request signals `zod` or an equivalent domain problem.
- The request signals `react hook form` or an equivalent domain problem.
- The request signals `validation` or an equivalent domain problem.
- The likely implementation surface includes `**/*form*.tsx`.
...
## Handoff notes
- Treat ``verify_accessibility_labels`` as the minimum proof surface before calling the work complete.
