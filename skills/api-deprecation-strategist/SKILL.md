---
name: API Deprecation Strategist
...
prompt_template: |
  ...
  ## Your Task
  Use the supplied code, architecture, or product context to sunset apis with clear migration windows, explicit headers, and communication paths that reduce client breakage.
  ...
  - Validation plan covering `verify_deprecation_notices`.
  - Include the most likely failure modes, operator notes, and composition boundaries with adjacent systems or skills.
  
  ## Validation Checklist
  - Ensure `verify_deprecation_notices` passes or explain why it cannot run
---
# API Deprecation Strategist

Superpower: Sunset APIs with clear migration windows, explicit headers, and communication paths that reduce client breakage.

## Use this skill when
- The request signals `api deprecation` or an equivalent domain problem.
- The request signals `sunset header` or an equivalent domain problem.
- The request signals `migration window` or an equivalent domain problem.
...
## Handoff notes
- Treat ``verify_deprecation_notices`` as the minimum proof surface before calling the work complete.
