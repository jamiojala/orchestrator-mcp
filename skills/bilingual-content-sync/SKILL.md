...
prompt_template: |
  ...
  ## Your Task
  Use the supplied code, architecture, or product context to maintain product and ui parity across languages while adapting tone and cultural context beyond literal translation.
  ...
  - Ensure `verify_translation_coverage` passes or explain why it cannot run
---
...
## Use this skill when
- The request signals `i18n` or an equivalent domain problem.
- The request signals `translation parity` or an equivalent domain problem.
- The request signals `localized copy` or an equivalent domain problem.
- The likely implementation surface includes `**/locales/**/*.json`.
...
## Handoff notes
- Treat ``verify_translation_coverage`` as the minimum proof surface before calling the work complete.
