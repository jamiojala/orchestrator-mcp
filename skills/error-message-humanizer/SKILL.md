...
prompt_template: |
  ...
  ## Your Task
  Use the supplied code, architecture, or product context to translate raw technical failures into useful user guidance with recovery paths and better emotional tone.
  ...
  - Validation plan covering `verify_error_recovery_rates`.
  - Include the most likely failure modes, operator notes, and composition boundaries with adjacent systems or skills.
  
  ## Validation Checklist
  - Ensure `verify_error_recovery_rates` passes or explain why it cannot run
---
...
## Use this skill when
- The request signals `error message` or an equivalent domain problem.
- The request signals `recovery` or an equivalent domain problem.
- The request signals `ux writing` or an equivalent domain problem.
- The likely implementation surface includes `**/*.tsx`.
- The likely implementation surface includes `**/*.ts`.
- The likely implementation surface includes `**/errors/**`.
...
## Handoff notes
- Treat ``verify_error_recovery_rates`` as the minimum proof surface before calling the work complete.
