...
prompt_template: |
  ...
  - Ensure `verify_completeness` passes or explain why it cannot run
---
...
- The request signals `exit readiness` or an equivalent domain problem.
- The likely implementation surface includes `**/*.md`.
- The likely implementation surface includes `**/legal/**`.
...
## Handoff notes
- Treat ``verify_completeness`` as the minimum proof surface before calling the work complete.
