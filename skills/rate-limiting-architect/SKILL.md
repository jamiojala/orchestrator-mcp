...
prompt_template: |
  ...
  ## Your Task
  Use the supplied code, architecture, or product context to implement distributed rate limiting with token buckets, predictable headers, and production-safe behavior under load.
  ...
  - Ensure `verify_rate_limit_headers` passes or explain why it cannot run
---
...
## Use this skill when
- The request signals `rate limit` or an equivalent domain problem.
- The request signals `token bucket` or an equivalent domain problem.
- The request signals `redis` or an equivalent domain problem.
...
## Handoff notes
- Treat ``verify_rate_limit_headers`` as the minimum proof surface before calling the work complete.
