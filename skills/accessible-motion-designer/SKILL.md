---
name: Accessible Motion Designer
...
prompt_template: |
  ...
  ## Your Task
  Use the supplied code, architecture, or product context to create cinematic motion systems with reduced-motion fallbacks, semantic announcements, and inclusive scroll choreography.
  ...
  - Validation plan covering `audit_accessibility_compliance`.
  - Include the most likely failure modes, operator notes, and composition boundaries with adjacent systems or skills.
  
  ## Validation Checklist
  - Ensure `audit_accessibility_compliance` passes or explain why it cannot run
---
...
## Use this skill when
- The request signals `reduced motion` or an equivalent domain problem.
- The request signals `scroll animation` or an equivalent domain problem.
- The request signals `accessibility` or an equivalent domain problem.
- The likely implementation surface includes `**/*.tsx`.
- The likely implementation surface includes `**/*.css`.
- The likely implementation surface includes `**/*scroll*.ts`.
...
## Handoff notes
- Treat ``audit_accessibility_compliance`` as the minimum proof surface before calling the work complete.
